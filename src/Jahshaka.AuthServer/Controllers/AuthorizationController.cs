...
namespace Jahshaka.AuthServer.Controllers
{
    public class AuthorizationController : Controller
    {
        ...
        public async Task<IActionResult> Exchange(OpenIdConnectRequest request)
        {
            ...
            if (request.GrantType == "urn:ietf:params:oauth:grant-type:external_account")
            {

                try
                {
                    foreach (var parameter in new List<string>() { "user_id", "concurrency_stamp" })
                    {
                        if (string.IsNullOrEmpty(request.GetParameter(parameter)?.ToString()))
                        {
                            return BadRequest(new OpenIdConnectResponse
                            {
                                Error = OpenIdConnectConstants.Errors.InvalidGrant,
                                ErrorDescription = $"The parameter '{parameter}' is required."
                            });
                        }
                    }

                    var userId = Guid.Parse(request.GetParameter("user_id")?.ToString());
                    var concurrencyStamp = request.GetParameter("concurrency_stamp")?.ToString();

                    var user = _applicationDbContext.Users
                        .FirstOrDefault(u => u.Id == userId && u.ConcurrencyStamp == concurrencyStamp);
                    ...
                    if (user == null)
                    {
                        ...
                        {
                            Error = OpenIdConnectConstants.Errors.InvalidGrant,
                            ErrorDescription = $"No user found with id {userId}"
                        });
                    }
                    ...
                    if (_userManager.SupportsUserLockout && await _userManager.IsLockedOutAsync(user))
                    {
                        ...
                        {
                            Error = OpenIdConnectConstants.Errors.InvalidGrant,
                            ErrorDescription = "The specified user account is locked."
                        });
                    }
                    ...
                    authenticationProperties.SetProperty("__user_id", user.Id.ToString());

                    var ticket = await CreateTicketAsync(request, user, authenticationProperties);      
                ...
                catch (Exception ex)
                {
                    ...
                    //await _alertService.SendCriticalAlertAsync(ex.GetFullMessage());
